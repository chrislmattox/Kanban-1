KanbanBoard = function(rallyDataSource, columnConfigShow) {
        ...
        var filterByTagsDropdown, showAgeAfter,
                ...
                hideLastColumnIfReleased, cosField, showTaskCompletion, showDefectStatus,
                lastState, artifactTypes,
                colorByArtifactType, policyCheckBox, classOfService;
    
        that._showThroughputReport = function() {
            var columnConfig = {reportNumber: 180, reportName: "Throughput Report",
                reportKanbanField: reportKanbanField, artifactTypes: artifactTypes};
            if (filterByTagsDropdown && filterByTagsDropdown.getValue()) {
                columnConfig.tags = [filterByTagsDropdown.getValue()];
                columnConfig.reportName += " filtered by " + filterByTagsDropdown.getDisplayedValue() + " tag";
            }
            var report = new KanbanReport(columnConfig, rallyDataSource);
            report.display("cycleTimeReportDiv");
        };
    
        that._showCycleTimeReport = function() {
            var columnConfig = {reportNumber: 170, reportName: "Cycle Time Report",
                reportKanbanField: reportKanbanField, artifactTypes: artifactTypes};
            if (filterByTagsDropdown && filterByTagsDropdown.getValue()) {
                columnConfig.tags = [filterByTagsDropdown.getValue()];
                columnConfig.reportName += " filtered by " + filterByTagsDropdown.getDisplayedValue() + " tag";
        ...
        that._createLayout = function(element) {
            ...
            var cosLegendContainerDiv = document.createElement("div");
            cosLegendContainerDiv.id = "cosLegendDivId";
            dojo.addClass(cosLegendContainerDiv, "cosLegendContainer");
            element.appendChild(cosLegendContainerDiv);
        ...
        that._getAndStorePrefData = function(callback) {
            function populateConfigForm(results) {
                ...
                if (kanbanInfo) {
                    ...
                    columns = {};
                    scheduleStatesMap = {};
                    classOfService = kanbanInfo.cosFieldInfos;
        ...
        that._redisplayBoard = function() {
            function displayBoard() {
                ...
                var cardboardConfig = {
                    ...
                    columns         : columns,
                    fetch           : "Name,FormattedID,Owner,ObjectID,Rank,Ready,Blocked,LastUpdateDate,Tags,State," + cosField
                };
    
                if (classOfService) {
					var cosLegendDiv = document.getElementById("cosLegendDivId");
					while(cosLegendDiv.childNodes.length >= 1) {
						cosLegendDiv.removeChild(cosLegendDiv.firstChild);
					} 
					var cosLegendTable = document.createElement('table');
					dojo.addClass(cosLegendTable, "cosLegendTable");
					
					var row = document.createElement('tr');
					var headerCell = document.createElement('td');
					
					var headerTextNode = document.createTextNode("Classes of Service:");
					headerCell.appendChild(headerTextNode);  
					
					var padSpan = document.createElement('span');
					dojo.style(padSpan, 'paddingLeft', '10px');
					headerCell.appendChild(padSpan);
					
					row.appendChild(headerCell);
					
					for (var key in classOfService) {
						var stateName;
						if (key) {
							stateName = key;
						}
						...
						var colorNumber = "#FFFFFF";
						if (classOfService[key].state) {
							 colorNumber = classOfService[key].state;
						}
						
						var stateCell = document.createElement('td');
						
						var cosColorSpan = document.createElement('span');
						dojo.addClass(cosColorSpan, 'cosColorSpan');
						dojo.style(cosColorSpan, 'backgroundColor', colorNumber);
						stateCell.appendChild(cosColorSpan);

						var padSpan2 = document.createElement('span');
						dojo.style(padSpan2, 'paddingLeft', '2px');
						stateCell.appendChild(padSpan2);
						
						var cosStateSpan = document.createElement('span');
						var cosStateTextNode = document.createTextNode(stateName);
						cosStateSpan.appendChild(cosStateTextNode);   
						stateCell.appendChild(cosStateSpan);

						var padSpan3 = document.createElement('span');
						dojo.style(padSpan3, 'paddingLeft', '10px');
						stateCell.appendChild(padSpan3);
						
						row.appendChild(stateCell);
					}
					// Don't render the table if only the header cell is in the row
					if (row.childNodes.length > 1) { 
						cosLegendTable.appendChild(row);
						cosLegendDiv.appendChild(cosLegendTable);
					}
				}                
